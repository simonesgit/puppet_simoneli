...
def main():
    ...
    print("\nSummary:")
    for node, result in results.items():
        print(f"{node} - Average Response Time: {result['average_response_time']:.2f}ms, Success: {result['success_count']}, Fail: {result['fail_count']}")
    ...
    with open(f"performance_test_report_{timestamp}.html", "w") as f:
        f.write("<html><head><title>Performance Test Result</title></head><body>")
