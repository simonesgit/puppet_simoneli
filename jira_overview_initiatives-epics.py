...
for index, row in overview_df.iterrows():
    ...
    if pd.notnull(e_target_end) and (pd.isnull(i_target_end) or (pd.notnull(i_target_end) and pd.notnull(e_target_end) and e_target_end > i_target_end)):
    ...
    if pd.notnull(i_due_date) and (pd.isnull(i_target_end) or (pd.notnull(i_target_end) and pd.notnull(i_due_date) and i_due_date > i_target_end)):
        overview_df.at[index, 'I_TargetEnd'] = i_due_date
        warn_message += f"I_Key: {row['I_key']}, I_duedate: {i_due_date}, "
    
    e_due_date = row['E_duedate']
    if pd.notnull(e_due_date) and (pd.isnull(e_target_end) or (pd.notnull(e_target_end) and pd.notnull(e_due_date) and e_due_date > e_target_end)):
