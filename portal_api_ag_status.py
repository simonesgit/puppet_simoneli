...
def main(is_production=False):
    ...
    with open('ag_diag_comm_results.csv', 'w', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=['server', 'agent', 'api_rc', 'api_msg', 'diag_comm', 'time_taken'])
        writer.writeheader()
        ...
        with tqdm(total=total_servers, desc='Progress') as pbar:
            ...
            for server in servers:
                try:
                    ...
                    diag_comm, api_msg, time_taken = api.get_agent_status(server['server'], server['agent'])
                    writer.writerow({
...
if __name__ == "__main__":
    is_production = False  # Set to True for productionor False for UAT
    main(is_production)
